    ...
    builder: async (yargs) => {
        for (let name of Object.keys(collection)) {
            ...
            yargs.command({
                ...
                builder: (yargs) => {
                    return yargs.default('outputPath', '.').options(getOptionsBySchema(getSchema(style)));
                },
                    ...
                    for (let i = 0; i < count; i++) {
                        queue.add(async () => {
                            ...
                            switch (format) {
                                ...
                                case 'png':
                                    await fs.writeFile(fileName, Buffer.from(await toPng(avatar).toArrayBuffer()));
                                    break;

                                case 'jpg':
                                case 'jpeg':
                                    await fs.writeFile(fileName, Buffer.from(await toJpeg(avatar).toArrayBuffer()));
                                    break;
