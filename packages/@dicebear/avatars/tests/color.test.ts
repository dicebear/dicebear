import legacy from '../dist';

test('Hex converter.', () => {
  let color = '#556677';

  expect(new legacy.color(color).rgb).toEqual([85, 102, 119]);
  expect(new legacy.color(color).rgba).toEqual([85, 102, 119, 1]);
  expect(new legacy.color(color).hsv).toEqual([210, 29, 47]);
});

test('Rgb converter.', () => {
  let color = 'rgb(85, 102, 119)';

  expect(new legacy.color(color).hex).toEqual('#556677');
  expect(new legacy.color(color).rgba).toEqual([85, 102, 119, 1]);
  expect(new legacy.color(color).hsv).toEqual([210, 29, 47]);
});
...
test('Hsv converter.', () => {
  ...
  expect(new legacy.color(color).rgb).toEqual([85, 102, 120]);
  expect(new legacy.color(color).rgba).toEqual([85, 102, 120, 1]);
  expect(new legacy.color(color).hex).toEqual('#556678');
});

test('Dark colors should become brighter.', () => {
  let color1 = new legacy.color('hsv(150, 150, 150)');
  let color2 = new legacy.color('hsv(150, 150, 150)');
...
test('Light colors should become darker.', () => {
  let color1 = new legacy.color('hsv(150, 150, 149)');
  let color2 = new legacy.color('hsv(150, 150, 150)');
...
test('Dark colors should become darker.', () => {
  let color1 = new legacy.color('hsv(150, 150, 151)');
  let color2 = new legacy.color('hsv(150, 150, 150)');
