...
export const create: StyleCreate<Options> = ({ prng, options }) => {
  ...
  // prettier-ignore
  const svg = [
    `<text x="50%" y="50%" font-family="${escapeXml(fontFamily)}" font-size="${escapeXml(`${fontSize}`)}" font-weight="${escapeXml(`${fontWeight}`)}" fill="${escapeXml(textColor)}" text-anchor="middle" dy="${(fontSize * .356).toFixed(3)}">${escapeXml(initials)}</text>`,
  ].join('');
