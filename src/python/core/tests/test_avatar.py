...
def test_rejects_a_raw_definition() -> None:
    with pytest.raises(TypeError, match=r"Avatar\(Style\(definition\), options\)"):
        Avatar(_minimal_style_data(), {"seed": "test"})


def test_accepts_raw_options_dict() -> None:
