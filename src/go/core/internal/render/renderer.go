...
// randomSuffix returns the hex string that separates this render from every
// other one under idRandomization, drawn once and cached. Process randomness
// is intentional — a PRNG-derived value would repeat for the same seed.
func (r *renderer) randomSuffix() string {
	if r.cachedRandomSuffix == nil {
		// % 0xffffff yields 0..0xfffffe, matching JS
		// Math.floor(random()*0xffffff) (exclusive upper bound) rather than a
		// masked 0..0xffffff.
		v := fmt.Sprintf("%06x", rand.Uint32()%0xffffff)
		r.cachedRandomSuffix = &v
	}
	return *r.cachedRandomSuffix
}

// randomizeIDs suffixes every id declaration and reference with the render's
// random suffix so that multiple instances of the same avatar do not collide
// in a shared document. The animation class and keyframe names carry the same
// suffix through animationHash.
func (r *renderer) randomizeIDs(svg string) string {
