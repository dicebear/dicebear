...
namespace DiceBear
{
    ...
    public static class Color
    {
        ...
        /// <summary>
        /// Returns a new list sorted by descending contrast against the
        /// reference color.
        /// </summary>
        /// <remarks>
        ...
        public static IReadOnlyList<string> SortByContrast(
            ...
            string refColor)
        {
            var refLuminance = Luminance(refColor);

            // OrderByDescending is a stable sort, like Array.prototype.sort in
            // the reference: candidates of equal contrast keep their relative
            // order. List.Sort would not.
            return candidates
                .Select(candidate =>
                {
                    var luminance = Luminance(candidate);
                    var ratio = (Math.Max(luminance, refLuminance) + 0.05)
                        / (Math.Min(luminance, refLuminance) + 0.05);

                    return new { Color = candidate, Ratio = ratio };
                })
        ...
        {
            var normalized = new HashSet<string>(excluded.Select(ToRgbHex), StringComparer.Ordinal);
            var filtered = candidates.Where(c => !normalized.Contains(ToRgbHex(c))).ToList();
        ...
        /// </summary>
        private static double Linearize(int channel) => Linearized[channel];

        /// <summary>
        /// Linear-light value for every 8-bit sRGB channel value, precomputed
        /// from <c>s &lt;= 0.04045 ? s / 12.92 : ((s + 0.055) / 1.055) ** 2.4</c>
        /// with <c>s = channel / 255</c>.
        /// </summary>
        /// <remarks>
        /// A lookup table because exponentiation is not required to be
        /// correctly rounded: results differ between JS engines and the math
        /// libraries used by the other language ports, which would break
        /// cross-language byte parity.
        /// </remarks>
        private static readonly double[] Linearized =
        {
        ...
        0.8307698767746546, 0.83879901174074, 0.846873231509858,
        0.8549926081242338, 0.8631572134541023, 0.8713671191987973,
        0.8796223968878317, 0.8879231178819663, 0.8962693533742664,
        0.9046611743911496, 0.9130986517934192, 0.9215818562772946,
        0.9301108583754237, 0.938685728457888, 0.9473065367331999,
        0.9559733532492861, 0.9646862478944651, 0.9734452903984125,
