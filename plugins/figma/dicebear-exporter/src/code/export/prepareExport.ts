...
export function prepareExport() {
  ...
  const exportData: Export = {
    frame: {
      ...
      settings: getFrameSettings(frameSelection),
    },
    components: {},
    colors: {},
  };
  ...
  while ((queueItem = queue.pop())) {
    for (let node of findAllNodesWithColor(queueItem)) {
      for (let color of getColorsByNode(node).values()) {
        ...
        if (undefined === exportData.colors[colorGroupName]) {
          const settings = getColorGroupSettings(colorGroupName);
          const colorGroup: ExportColorGroup = (exportData.colors[
            colorGroupName
          ] = {
            settings: {
              ...settings,
          ...
          for (const [colorName, color] of colorGroups.get(colorGroupName)) {
            const solidPaint = color.paints[0] as SolidPaint;

            colorGroup.collection[colorName] = {
              id: color.id,
              name: color.name,
              value: {
            ...
            colorGroup.settings.defaults[colorName] =
