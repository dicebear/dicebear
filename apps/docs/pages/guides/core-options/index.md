...
Only the way you pass them differs from one language to the next, so each library
page shows that in its own syntax. The names, types, defaults, and behavior below
do not change.

They apply to every avatar style. Where the type lists `[min, max]`, you may pass
either a fixed value or a two-element tuple. The PRNG samples a value from the
tuple's range.
...
| `tags`            | `string \| string[]`                             | _unset_       | Keep only variants matching these [tags](/guides/filter-variants-with-tags/) (`category:value`, prefix with `!` to exclude) |
...
such as `mood:happy` or `hairLength:long`. Within one category the values combine
with "or", different categories combine with "and", and a leading `!` excludes.
