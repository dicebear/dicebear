...
The look comes from [avatar styles](/styles/). There are %STYLE_COUNT% of
them, drawn by different artists, ranging from abstract shapes to illustrated
...
away. Every style has options such as background color, flip,
or scale, and most add their own, like hair, eyes, or accessories. So an
avatar is the answer to three questions: which style, which seed, which
options. Curious what happens under the hood?
[How avatars are made](/understand/how-avatars-are-made/) walks through it.

## Where it runs

Anywhere, in practice. Libraries for JavaScript, PHP, Python, Rust, Go, Dart,
and C# generate avatars directly in your code. The free HTTP API serves them
by URL for everything else. A CLI covers batch exports, and the
...
The libraries generate avatars entirely on your infrastructure, so no data
about your users leaves your systems. If you want the URL-based workflow with
the same control, the HTTP API is open source and
