...
async function download(format: 'svg' | 'png' | 'jpg') {
    ...
    const response = await fetch(getAvatarApiUrl(store.avatarStyleName, options.value, format));
    ...
    <v-dialog v-model="open" width="540">
        <v-card>
            ...
            <div class="dialog-preview">
                <Avatar :style-name="store.avatarStyleName" :style-options="options" :size="96" />
            </div>
            <v-card-title class="dialog-title"> Your avatar will be downloaded! 🎉 </v-card-title>
            <v-card-text class="dialog-text">
                <v-alert type="info" variant="tonal" class="dialog-license">
                    <strong> Please note the following license before using the avatar: </strong>
